...
export default function LabsNewsletter() {
  ...
  return (
    <div style={{ border: '1px solid rgba(127,149,255,0.3)', borderRadius: 12, padding: '26px 24px', background: 'var(--lw-panel-accent)' }}>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 13, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--lw-accent-soft)', marginBottom: 10 }}>
        Subscribe
      </div>
      <h2 style={{ fontSize: 24, fontWeight: 500, letterSpacing: '-0.01em', margin: '0 0 8px', color: 'var(--lw-fg)' }}>
        New research in your inbox
      </h2>
      <p style={{ fontSize: 16, color: 'var(--lw-muted)', lineHeight: 1.6, margin: '0 0 18px', maxWidth: 460 }}>
      ...
      <form onSubmit={submit} style={{ display: 'flex', gap: 10, flexWrap: 'wrap', maxWidth: 460 }}>
        ...
        <input
          ...
          style={{ flex: 1, minWidth: 200, background: 'var(--lw-panel)', border: '1px solid var(--lw-line-2)', borderRadius: 6, padding: '11px 13px', color: 'var(--lw-fg)', fontSize: 16, fontFamily: 'inherit' }}
        />
        <button
          type="submit"
          disabled={isLoading || isSuccess}
          style={{ fontFamily: 'var(--font-mono)', fontSize: 14, letterSpacing: '0.06em', background: 'var(--lw-fg)', color: 'var(--lw-bg)', padding: '11px 18px', borderRadius: 6, border: 'none', cursor: isLoading || isSuccess ? 'default' : 'pointer', whiteSpace: 'nowrap', opacity: isLoading ? 0.6 : 1 }}
        >
